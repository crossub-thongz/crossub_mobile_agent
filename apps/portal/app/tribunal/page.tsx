...
export default function TribunalPage() {
  ...
  const [openAfterRefreshId, setOpenAfterRefreshId] = useState<string | null>(null);

  useEffect(() => {
    if (!openAfterRefreshId) return;
    const created = tribunalCases.find((row) => row.id === openAfterRefreshId);
    if (!created) return;
    openJob(tribunalToJobRow(created));
    setOpenAfterRefreshId(null);
  }, [openAfterRefreshId, openJob, tribunalCases]);
